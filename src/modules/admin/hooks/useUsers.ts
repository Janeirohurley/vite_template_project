...
import { getUsersApi, getUserByIdApi, createUserApi, updateUserApi, deleteUserApi, bulkDeleteUsersApi, getRolesApi, uploadProfilePictureApi, setupUserTotp2faApi, setupUserStatic2faApi, setupUserEmail2faApi } from '../api/users';
...
/**
 * Admin: Setup TOTP 2FA for a user
 */
export function useSetupUserTotp2fa() {
    const queryClient = useQueryClient();
    return useMutation<{ email: string; qr_code: string; config_url: string }, ApiError, string>({
        mutationFn: (id) => setupUserTotp2faApi(id),
        onSuccess: (_, userId) => {
...
/**
 * Admin: Setup static 2FA for a user
 */
export function useSetupUserStatic2fa() {
    const queryClient = useQueryClient();
    return useMutation<{ email: string; backup_codes: string[] }, ApiError, string>({
        mutationFn: (id) => setupUserStatic2faApi(id),
        onSuccess: (_, userId) => {
            queryClient.invalidateQueries({ queryKey: ['users', userId] });
            queryClient.invalidateQueries({ queryKey: ['users'] });
        },
 ...
 */
export function useSetupUserEmail2fa() {
    const queryClient = useQueryClient();
    return useMutation<{ email: string }, ApiError, string>({
        mutationFn: (id) => setupUserEmail2faApi(id),
        onSuccess: (_, userId) => {
            queryClient.invalidateQueries({ queryKey: ['users', userId] });
            queryClient.invalidateQueries({ queryKey: ['users'] });
        },
