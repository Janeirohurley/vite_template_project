...
import { useUsers, useUpdateUser, useDeleteUser, useBulkDeleteUsers, useUploadProfilePicture, useSetupUserTotp2fa, useSetupUserStatic2fa, useSetupUserEmail2fa } from "../hooks/useUsers";
...
export function UsersManagementPage() {
    ...
    const setupTotp2faMutation = useSetupUserTotp2fa();
    const setupStatic2faMutation = useSetupUserStatic2fa();
    const setupEmail2faMutation = useSetupUserEmail2fa();
    ...
    const [staticSetupModal, setStaticSetupModal] = useState<{
        email: string;
        backupCodes: string[];
    } | null>(null);
    ...
    const handleCellEdit = async (rowId: string | number, columnKey: string, newValue: string) => {
        try {
            // Mise ? jour optimiste: on met ? jour imm?diatement l'UI
            // React Query va g?rer l'invalidation et le rechargement

            if (columnKey === "role") {
                columnKey = "role_id"
            }
            let normalizedValue: string | boolean = newValue;
            if (booleanFields.has(columnKey)) {
                normalizedValue = toBoolean(newValue);
            }

            // Admin 2FA setup flows to return QR / backup codes
            if (columnKey === "requires_2fa_qr" && normalizedValue === true) {
                const result = await setupTotp2faMutation.mutateAsync(String(rowId));
                setTotpSetupModal({
                    email: result.email,
            ...
            if (columnKey === "requires_2fa_static" && normalizedValue === true) {
                const result = await setupStatic2faMutation.mutateAsync(String(rowId));
                setStaticSetupModal({
                    email: result.email,
            ...
            if (columnKey === "requires_2fa_email" && normalizedValue === true) {
                await setupEmail2faMutation.mutateAsync(String(rowId));
                return;
            }

            await updateUserMutation.mutateAsync({
                id: String(rowId),
                userData: { [columnKey]: normalizedValue }
            });
        } catch (error) {
            alert("Erreur lors de la mise ? jour de l'utilisateur");
            throw new Error("Erreur lors de la mise ? jour de l'utilisateur", { cause: error });
        }
    };
            ...
            <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                        Utilisateur: {totpSetupModal.email}
                    </p>
                    <div className="flex justify-center mb-4">
                        <img
                            src={totpSetupModal.qrCode}
                            alt="QR code 2FA"
                            className="w-48 h-48 rounded-md border"
                        />
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-800 rounded p-3 text-xs break-all mb-4">
                        {totpSetupModal.configUrl}
                    </div>
                    <div className="flex gap-2 justify-end">
                        <Button
                            variant="outline"
                            onClick={() => {
                                navigator.clipboard.writeText(totpSetupModal.configUrl);
                                notify.success("Lien copié");
                            }}
                        >
                            Copier lien
                        </Button>
                        <Button onClick={() => setTotpSetupModal(null)}>Fermer</Button>
                    </div>
                </div>
            </div>
        )}
        ...
        {staticSetupModal && (
            <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                        Utilisateur: {staticSetupModal.email}
                    </p>
                    <div className="bg-gray-50 dark:bg-gray-800 rounded p-3 text-sm mb-4">
                        {staticSetupModal.backupCodes.map(code => (
                    ...
                    <div className="flex gap-2 justify-end">
                        <Button
                            variant="outline"
                            onClick={() => {
                                navigator.clipboard.writeText(staticSetupModal.backupCodes.join("\n"));
                                notify.success("Codes copiés");
                            }}
                        >
                            Copier
                        </Button>
                        <Button onClick={() => setStaticSetupModal(null)}>Fermer</Button>
                    </div>
                </div>
            </div>
        )}
