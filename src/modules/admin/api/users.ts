...
/**
 * Admin: Setup TOTP 2FA for a user (returns QR code)
 */
export async function setupUserTotp2faApi(id: string): Promise<{ email: string; qr_code: string; config_url: string }> {
    try {
        const response = await axios.post<DjangoSuccessResponse<{ email: string; qr_code: string; config_url: string }>>(
            `/users/${id}/admin-2fa/totp/`
        );
        return response.data.data as { email: string; qr_code: string; config_url: string };
    } catch (error: unknown) {
        const axiosError = error as { formattedError?: ApiError };
        if (axiosError.formattedError) {
            throw axiosError.formattedError;
...
/**
 * Admin: Setup static 2FA for a user (returns backup codes)
 */
export async function setupUserStatic2faApi(id: string): Promise<{ email: string; backup_codes: string[] }> {
    try {
        const response = await axios.post<DjangoSuccessResponse<{ email: string; backup_codes: string[] }>>(
            `/users/${id}/admin-2fa/static/`
        );
        return response.data.data as { email: string; backup_codes: string[] };
    } catch (error: unknown) {
        const axiosError = error as { formattedError?: ApiError };
        if (axiosError.formattedError) {
            throw axiosError.formattedError;
...
/**
 * Admin: Enable email 2FA for a user
 */
export async function setupUserEmail2faApi(id: string): Promise<{ email: string }> {
    try {
        const response = await axios.post<DjangoSuccessResponse<{ email: string }>>(
            `/users/${id}/admin-2fa/email/`
        );
        return response.data.data as { email: string };
    } catch (error: unknown) {
        const axiosError = error as { formattedError?: ApiError };
        if (axiosError.formattedError) {
            throw axiosError.formattedError;
