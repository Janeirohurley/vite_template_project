...
import { saveInscriptionDraft, markDraftAsCompleted, generateSessionId } from '../../../lib/inscriptionDraftService';
...
export function InscriptionWizard({ isOpen, onClose, onComplete, resumeDraft }: InscriptionWizardProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (resumeDraft) {
        ...
        setBackendId(resumeDraft.backendId);
  ...
  const handleClose = async () => {
    if (sessionId && Object.keys(formData).length > 0) {
      await saveInscriptionDraft(sessionId, currentStep, formData, undefined, backendId);
    }
    onClose();
  };
  ...
  const handleNext = async (stepData: any) => {
    ...
    if (currentStep === 6) {
      ...
      await saveInscriptionDraft(sessionId, nextStep, updatedData, undefined, backendId);
    } else {
      if (currentStep === 3 && !stepData.has_university_background) {
        setCurrentStep(5);
        await saveInscriptionDraft(sessionId, 5, updatedData, undefined, backendId);
      } else {
  ...
  const handlePrevious = async () => {
    ...
    await saveInscriptionDraft(sessionId, prevStep, formData, undefined, backendId);
  };
