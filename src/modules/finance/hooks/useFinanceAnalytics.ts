  ...
  const stats = useMemo(() => {
    ...
    try {
      ...
      const totalExpected = paymentPlans.reduce((acc, plan) => {
        return acc + (Number(plan.amount) || 0);
      }, 0);
      ...
      const monthlyFlow = Array.from({ length: 6 }, (_, i) => {
        ...
        const total = payments
          .filter(p => {
            try {
              const paymentDate = new Date(p.payment_date || p.created_at || 0);
      ...
      const paymentMethods = [
        { method: 'Espèces', amount: payments.filter(p => p.payment_method === 'cash').reduce((a, p) => a + (Number(p.amount) || 0), 0) },
        { method: 'Virement', amount: payments.filter(p => p.payment_method === 'transfer').reduce((a, p) => a + (Number(p.amount) || 0), 0) },
        { method: 'Chèque', amount: payments.filter(p => p.payment_method === 'check').reduce((a, p) => a + (Number(p.amount) || 0), 0) }
      ].filter(m => m.amount > 0);
